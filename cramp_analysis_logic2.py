...
def detect_bursts_percentile(emg_rms, percentile=75, min_duration_sec=0.5, fs=20):
    """Standard burst detection: looks for rises above threshold. Use for baseline."""
...
def detect_bursts_saturated(emg_rms, dip_threshold_v=4.7, min_duration_sec=0.5, fs=20):
    """
    Inverted burst detection for saturated signals.
    The signal is near 5V (ceiling) most of the time.
    Bursts are identified as periods ABOVE dip_threshold_v,
    separated by dips below it (which represent inter-burst gaps).
    """
    above = emg_rms > dip_threshold_v
    bursts = []
    in_burst = False
    burst_start = 0
    ...
    return bursts, dip_threshold_v

baseline_bursts, baseline_threshold = detect_bursts_percentile(baseline['rms'].values, fs=fs)

# Use saturated burst detection for cramping signal
cramping_bursts, cramping_threshold = detect_bursts_saturated(
    cramping['rms'].values, dip_threshold_v=4.70, min_duration_sec=5.0, fs=fs)
print("\nNote: Cramping burst detection uses inverted (saturation-aware) method.")
...
plt.tight_layout()
plt.savefig('emg_cramp_analysis_2.png', dpi=300, bbox_inches='tight')
...
results_df.to_csv('myoware_results/emg_analysis_results_2.csv', index=False)